...
var bossdone = true #eventtrigger
...
func _ready():
	...
	_change_player_name(settings.globalSetting["player_identifier"])
...
func _on_leaderboard_request_complete(result, response_code, headers, body):
	...
	for n in json.get_data().items.size():
		...
		int(json.get_data().items[n].score)))
